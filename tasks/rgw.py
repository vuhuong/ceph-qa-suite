...
def configure_regions_and_zones(ctx, config, regions, role_endpoints, realm):
    ...
    for role, (master_client, zone, zone_info, user_info) in role_zones.iteritems():
        ...
        rgwadmin(ctx, master_client,
                 cmd=['-n', master_client, 'zone', 'set', '--rgw-zone', zone],
    ...
    for region, info in region_info.iteritems():
        ...
        rgwadmin(ctx, master_client,
                 cmd=['-n', master_client, 'region', 'set'],
        ...
        if client != master_client:
            rgwadmin(ctx, client,
                cmd=['-n', client, 'realm', 'pull', '--rgw-realm', realm, '--url',
                     region_info[master_region]['info']['endpoints']],
                     check_status=True)
